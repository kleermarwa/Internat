...
$sql = "SELECT decharge.*, students.*
        ...
        WHERE decharge.status = 'pending' AND decharge.valide_departement = 1 AND decharge.valide_internat = 1 AND decharge.valide_economique = 0";
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $output .= "<td><a class='validateDecharge' href='../admin/economique_validation.php?request_id=" . $row['id_demande'] . "&amp;name=" . urlencode($row['name']) . "'>Valider</a></td>";
        $output .= "</tr>";
    }
