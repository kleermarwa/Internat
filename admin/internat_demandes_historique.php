...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (strpos($previous_page, 'internat_demandes_valide.php') !== false) {
            $output .= "<td><button style='border-left: none; font-size:0.8rem' class='add-student blue' data-id='" . $row['id_demande'] . "' data-room-alias='" . $row['room_alias'] . "' data-student-id='" . $row['id'] . "' data-genre='" . $row['genre'] . "'>Ajouter chambre</button></td>";
            // Button to cancel validation
            $output .= "<td><button style='border-right: none; font-size:0.8rem' class='cancel-validation reject' data-id='" . $row['id_demande'] . "'>Annuler validation</button></td>";
        } elseif (strpos($previous_page, 'internat_demandes_refuse.php') !== false) {
